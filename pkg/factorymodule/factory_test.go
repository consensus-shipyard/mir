...
func (em *echoModule) applyEvent(event events.Event) (*events.EventList, error) {
	...
	switch e := event.(type) {
	case *events.TestStringEvent:
		return events.ListOf(events.NewTestString(destModuleID.Top(), em.prefix+e.Value)), nil
	case *eventpb.Event:
		switch e.Type.(type) {
		case *eventpb.Event_Init:
			return events.ListOf(events.NewTestString(destModuleID.Top(), string(em.id)+" Init")), nil //nolint:goconst
		default:
	...
	testCases := map[string]func(t *testing.T){
		...
		"00 Instantiate": func(t *testing.T) {
			...
			assert.Equal(t,
				...
				evOut.Slice()[0].(*events.TestStringEvent).Value,
			)
		},

		"01 Invoke": func(t *testing.T) {
			evOut, err := echoFactory.ApplyEvents(events.ListOf(events.NewTestString(
				echoFactoryID.Then("inst0"),
				"Hi!"),
			))
			assert.NoError(t, err)
		...
		"03 Invoke many": func(t *testing.T) {
			...
			for i := 5; i >= 0; i-- {
				evList.PushBack(events.NewTestString(
					echoFactoryID.Then(tp.ModuleID(fmt.Sprintf("inst%d", i))),
					"Hi!"),
				)
			}
			...
			sort.Slice(sortedOutput, func(i, j int) bool {
				return sortedOutput[i].(*events.TestStringEvent).Value <
					sortedOutput[j].(*events.TestStringEvent).Value
			})
			...
			for i := 0; i <= 5; i++ {
				...
				assert.Equal(t,
					...
					sortedOutput[i].(*events.TestStringEvent).Value,
				)
			}
		},

		"04 Wrong event type": func(t *testing.T) {
			wrongEvent := events.NewTestUint(echoFactoryID, 42)
		...
		"05 Wrong submodule ID": func(t *testing.T) {
			wrongEvent := events.NewTestUint(echoFactoryID.Then("non-existent-module"), 42)
			evOut, err := echoFactory.ApplyEvents(events.ListOf(wrongEvent))
			assert.NoError(t, err)
		...
		"07 Invoke garbage-collected": func(t *testing.T) {
			...
			for i := 3; i <= 5; i++ {
				...
				assert.Equal(t,
					fmt.Sprintf("Inst %d: Hi!", i),
					sortedOutput[i-3].(*events.TestStringEvent).Value,
				)
			}
