...
func Init(dest string) *eventpb.Event {
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_Init{Init: &eventpb.Init{}}}
}
...
func SendMessage(dest string, message *messagepb.Message, destinations []t.NodeID) *eventpb.Event {

	// TODO: This conversion can potentially be very inefficient!

	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_SendMessage{SendMessage: &eventpb.SendMessage{
...
func MessageReceived(dest string, from t.NodeID, message *messagepb.Message) *eventpb.Event {
...
func ClientRequest(dest string, clientID t.ClientID, reqNo t.ReqNo, data []byte, authenticator []byte) *eventpb.Event {
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_Request{Request: &requestpb.Request{
...
func HashRequest(dest string, data [][][]byte, origin *eventpb.HashOrigin) *eventpb.Event {
	...
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_HashRequest{HashRequest: &eventpb.HashRequest{
		Data:   hashData,
		Origin: origin,
	}}}
}
...
func HashResult(dest string, digests [][]byte, origin *eventpb.HashOrigin) *eventpb.Event {
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_HashResult{HashResult: &eventpb.HashResult{
		Digests: digests,
		Origin:  origin,
	}}}
}
...
func SignRequest(dest string, data [][]byte, origin *eventpb.SignOrigin) *eventpb.Event {
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_SignRequest{SignRequest: &eventpb.SignRequest{
		Data:   data,
		Origin: origin,
	}}}
}
...
func SignResult(dest string, signature []byte, origin *eventpb.SignOrigin) *eventpb.Event {
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_SignResult{SignResult: &eventpb.SignResult{
		Signature: signature,
		Origin:    origin,
	}}}
}
...
func RequestReady(dest string, requestRef *requestpb.RequestRef) *eventpb.Event {
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_RequestReady{RequestReady: &eventpb.RequestReady{
...
func WALAppend(dest string, event *eventpb.Event, retentionIndex t.WALRetIndex) *eventpb.Event {
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_WalAppend{WalAppend: &eventpb.WALAppend{
		Event:          event,
...
// specified one.
func WALTruncate(dest string, retentionIndex t.WALRetIndex) *eventpb.Event {
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_WalTruncate{WalTruncate: &eventpb.WALTruncate{
		RetentionIndex: retentionIndex.Pb(),
	}}}
}
...
func WALEntry(dest string, persistedEvent *eventpb.Event, retentionIndex t.WALRetIndex) *eventpb.Event {
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_WalEntry{WalEntry: &eventpb.WALEntry{
		Event: persistedEvent,
	}}}
}

// Deliver returns an event of delivering a request batch to the application in sequence number order.
func Deliver(dest string, sn t.SeqNr, batch *requestpb.Batch) *eventpb.Event {
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_Deliver{Deliver: &eventpb.Deliver{
		Sn:    sn.Pb(),
		Batch: batch,
	}}}
}
...
func VerifyRequestSig(dest string, reqRef *requestpb.RequestRef, signature []byte) *eventpb.Event {
	return &eventpb.Event{
		Destination: dest,
...
func RequestSigVerified(dest string, reqRef *requestpb.RequestRef, valid bool, error string) *eventpb.Event {
	return &eventpb.Event{
		Destination: dest,
...
func StoreVerifiedRequest(dest string, reqRef *requestpb.RequestRef, data []byte, authenticator []byte) *eventpb.Event {
	return &eventpb.Event{
		Destination: dest,
...
func AppSnapshotRequest(dest string, module string, epoch t.EpochNr) *eventpb.Event {
	return &eventpb.Event{
		Destination: dest,
...
func AppSnapshot(dest string, epoch t.EpochNr, data []byte) *eventpb.Event {
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_AppSnapshot{AppSnapshot: &eventpb.AppSnapshot{
		Epoch: epoch.Pb(),
		Data:  data,
	}}}
}

// AppRestoreState returns an event representing the protocol module
// asking the application for restoring its state from the snapshot.
func AppRestoreState(dest string, snapshot []byte) *eventpb.Event {
	return &eventpb.Event{
		Destination: dest,
...
func TimerDelay(dest string, events []*eventpb.Event, delay t.TimeDuration) *eventpb.Event {
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_TimerDelay{TimerDelay: &eventpb.TimerDelay{
		Events: events,
		Delay:  delay.Pb(),
	}}}
}

func TimerRepeat(dest string, events []*eventpb.Event, delay t.TimeDuration, retIndex t.TimerRetIndex) *eventpb.Event {
	return &eventpb.Event{Destination: dest, Type: &eventpb.Event_TimerRepeat{TimerRepeat: &eventpb.TimerRepeat{
		Events:         events,
		Delay:          delay.Pb(),
...
func TimerGarbageCollect(dest string, retIndex t.TimerRetIndex) *eventpb.Event {
	return &eventpb.Event{
		Destination: dest,
...
func PersistDummyBatch(dest string, sn t.SeqNr, batch *requestpb.Batch) *eventpb.Event {
	return &eventpb.Event{
		Destination: dest,
		Type: &eventpb.Event_PersistDummyBatch{PersistDummyBatch: &eventpb.PersistDummyBatch{
			Sn:    sn.Pb(),
			Batch: batch,
		}},
	}
...
func AnnounceDummyBatch(dest string, sn t.SeqNr, batch *requestpb.Batch) *eventpb.Event {
	return &eventpb.Event{
		Destination: dest,
		Type: &eventpb.Event_AnnounceDummyBatch{AnnounceDummyBatch: &eventpb.AnnounceDummyBatch{
			Sn:    sn.Pb(),
			Batch: batch,
		}},
	}
...
func StoreDummyRequest(dest string, reqRef *requestpb.RequestRef, data []byte) *eventpb.Event {
	return &eventpb.Event{
		Destination: dest,
		Type: &eventpb.Event_StoreDummyRequest{StoreDummyRequest: &eventpb.StoreDummyRequest{
			RequestRef: reqRef,
			Data:       data,
		}},
	}
