...
import (
	...
	types24 "github.com/filecoin-project/mir/codegen/model/types"
	...
	types21 "github.com/filecoin-project/mir/pkg/pb/blockchainpb/communicationpb/types"
	types19 "github.com/filecoin-project/mir/pkg/pb/blockchainpb/minerpb/types"
	types22 "github.com/filecoin-project/mir/pkg/pb/blockchainpb/synchronizerpb/types"
	types20 "github.com/filecoin-project/mir/pkg/pb/blockchainpb/tpmpb/types"
	...
	types23 "github.com/filecoin-project/mir/pkg/pb/testerpb/types"
	...
	types25 "github.com/filecoin-project/mir/pkg/timer/types"
	types26 "github.com/filecoin-project/mir/pkg/trantor/types"
...
func Event_TypeFromPb(pb eventpb.Event_Type) Event_Type {
	...
	case *eventpb.Event_Tpm:
		return &Event_Tpm{Tpm: types20.EventFromPb(pb.Tpm)}
	case *eventpb.Event_Communication:
		return &Event_Communication{Communication: types21.EventFromPb(pb.Communication)}
	case *eventpb.Event_Synchronizer:
		return &Event_Synchronizer{Synchronizer: types22.EventFromPb(pb.Synchronizer)}
	...
	case *eventpb.Event_Tester:
		return &Event_Tester{Tester: types23.TesterFromPb(pb.Tester)}
	}
	return nil
}
...
func (w *Event_Tpm) Unwrap() *types20.Event {
	return w.Tpm
}

func (w *Event_Tpm) Pb() eventpb.Event_Type {
	if w == nil {
		return nil
	}
	if w.Tpm == nil {
		return &eventpb.Event_Tpm{}
	}
	return &eventpb.Event_Tpm{Tpm: (w.Tpm).Pb()}
}

func (*Event_Tpm) MirReflect() mirreflect.Type {
	return mirreflect.TypeImpl{PbType_: reflectutil.TypeOf[*eventpb.Event_Tpm]()}
}

type Event_Communication struct {
	Communication *types21.Event
}

func (*Event_Communication) isEvent_Type() {}

func (w *Event_Communication) Unwrap() *types21.Event {
...
type Event_Synchronizer struct {
	Synchronizer *types22.Event
}

func (*Event_Synchronizer) isEvent_Type() {}

func (w *Event_Synchronizer) Unwrap() *types22.Event {
...
type Event_Tester struct {
	Tester *types23.Tester
}

func (*Event_Tester) isEvent_Type() {}

func (w *Event_Tester) Unwrap() *types23.Tester {
	return w.Tester
}
...
func EventFromPb(pb *eventpb.Event) *Event {
	...
	return &Event{
		...
		Next: types24.ConvertSlice(pb.Next, func(t *eventpb.Event) *Event {
...
func (m *Event) Pb() *eventpb.Event {
	...
	pbMessage := &eventpb.Event{}
	{
		...
		pbMessage.Next = types24.ConvertSlice(m.Next, func(t *Event) *eventpb.Event {
			return (t).Pb()
		})
	}
...
func TimerDelayFromPb(pb *eventpb.TimerDelay) *TimerDelay {
	...
	return &TimerDelay{
		EventsToDelay: types24.ConvertSlice(pb.EventsToDelay, func(t *eventpb.Event) *Event {
			return EventFromPb(t)
		}),
		Delay: (types25.Duration)(pb.Delay),
	}
}
...
func (m *TimerDelay) Pb() *eventpb.TimerDelay {
	...
	{
		pbMessage.EventsToDelay = types24.ConvertSlice(m.EventsToDelay, func(t *Event) *eventpb.Event {
			return (t).Pb()
		})
...
type TimerRepeat struct {
	...
	Delay          types25.Duration
	RetentionIndex types26.RetentionIndex
}
...
func TimerRepeatFromPb(pb *eventpb.TimerRepeat) *TimerRepeat {
	...
	return &TimerRepeat{
		EventsToRepeat: types24.ConvertSlice(pb.EventsToRepeat, func(t *eventpb.Event) *Event {
			return EventFromPb(t)
		}),
		Delay:          (types25.Duration)(pb.Delay),
...
func (m *TimerRepeat) Pb() *eventpb.TimerRepeat {
	...
	{
		pbMessage.EventsToRepeat = types24.ConvertSlice(m.EventsToRepeat, func(t *Event) *eventpb.Event {
			return (t).Pb()
		})
		pbMessage.Delay = (uint64)(m.Delay)
