...
import (
	...
	types16 "github.com/filecoin-project/mir/codegen/model/types"
	...
	types15 "github.com/filecoin-project/mir/pkg/pb/testerpb/types"
	...
	types17 "github.com/filecoin-project/mir/pkg/timer/types"
	types18 "github.com/filecoin-project/mir/pkg/trantor/types"
...
func Event_TypeFromPb(pb eventpb.Event_Type) Event_Type {
	...
	case *eventpb.Event_Tester:
		return &Event_Tester{Tester: types15.TesterFromPb(pb.Tester)}
	}
	return nil
}
...
type Event_Tester struct {
	Tester *types15.Tester
}

func (*Event_Tester) isEvent_Type() {}

func (w *Event_Tester) Unwrap() *types15.Tester {
	return w.Tester
}
...
func EventFromPb(pb *eventpb.Event) *Event {
	...
	return &Event{
		...
		Next: types16.ConvertSlice(pb.Next, func(t *eventpb.Event) *Event {
...
func (m *Event) Pb() *eventpb.Event {
	...
	pbMessage := &eventpb.Event{}
	{
		...
		pbMessage.Next = types16.ConvertSlice(m.Next, func(t *Event) *eventpb.Event {
			return (t).Pb()
		})
	}
...
func TimerDelayFromPb(pb *eventpb.TimerDelay) *TimerDelay {
	...
	return &TimerDelay{
		EventsToDelay: types16.ConvertSlice(pb.EventsToDelay, func(t *eventpb.Event) *Event {
			return EventFromPb(t)
		}),
		Delay: (types17.Duration)(pb.Delay),
	}
}
...
func (m *TimerDelay) Pb() *eventpb.TimerDelay {
	...
	{
		pbMessage.EventsToDelay = types16.ConvertSlice(m.EventsToDelay, func(t *Event) *eventpb.Event {
			return (t).Pb()
		})
...
type TimerRepeat struct {
	...
	Delay          types17.Duration
	RetentionIndex types18.RetentionIndex
}
...
func TimerRepeatFromPb(pb *eventpb.TimerRepeat) *TimerRepeat {
	...
	return &TimerRepeat{
		EventsToRepeat: types16.ConvertSlice(pb.EventsToRepeat, func(t *eventpb.Event) *Event {
			return EventFromPb(t)
		}),
		Delay:          (types17.Duration)(pb.Delay),
		RetentionIndex: (types18.RetentionIndex)(pb.RetentionIndex),
	}
}
...
func (m *TimerRepeat) Pb() *eventpb.TimerRepeat {
	...
	{
		pbMessage.EventsToRepeat = types16.ConvertSlice(m.EventsToRepeat, func(t *Event) *eventpb.Event {
			return (t).Pb()
		})
		pbMessage.Delay = (uint64)(m.Delay)
