...
func UponISSEvent[W types.ISSEvent_TypeWrapper[Ev], Ev any](m dsl.Module, handler func(ev *Ev) error) {
	dsl.UponMirEvent[*types1.Event_Iss](m, func(ev *types.ISSEvent) error {
		w, ok := ev.Type.(W)
		if !ok {
			return nil
		}
...
func UponPushCheckpoint(m dsl.Module, handler func() error) {
	UponISSEvent[*types.ISSEvent_PushCheckpoint](m, func(ev *types.PushCheckpoint) error {
		return handler()
	})
}
...
func UponSBDeliver(m dsl.Module, handler func(sn types2.SeqNr, data []uint8, aborted bool, leader types2.NodeID, instanceId types2.ModuleID) error) {
	UponISSEvent[*types.ISSEvent_SbDeliver](m, func(ev *types.SBDeliver) error {
