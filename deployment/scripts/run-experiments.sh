...
for exp_dir in "$dir"/[0-9][0-9][0-9][0-9]; do
  ...
  if [[ -f "$exp_dir/results.json" ]]; then
    ...
    ansible-playbook -i "$dir/hosts" --forks "$num_hosts" --extra-vars "node_config_file=$exp_dir/$node_config_file_name exp_id=$exp_id output_dir='$exp_dir' bench_output_file=$bench_output_file" run-benchmark.yaml || exit 1

    # Process the results of the experiment.
    python3 scripts/analyze-bench-output.py "$exp_id" "$exp_dir/$node_config_file_name" "$exp_dir/results.json" "$exp_dir"/data/*$node_output_file.json || exit 1

    # Aggregate data from this and all previous experiments in a single table.
    python3 scripts/aggregate-data.py "$dir"/partial-results.csv "$dir"/[0-9][0-9][0-9][0-9]/results.json > /dev/null || exit 1
  fi
done
