...
def printLines(data):
    for f, line in splitLine(data):
        ...
        print(f)
        for dp in line:
            print(dp["experiment-id"], dp[sortField], dp[xField], dp[yField])
...
def plotLine(ax, line, label):
    ...
    # print([dp["experiment-id"] for dp in line])
    # print([dp[xField] for dp in line])
    # print([dp[yField] for dp in line])
    d = [
        {"x": float(dp[xField]),
         "y": float(dp[yField]),
         "experiment-id": dp["experiment-id"]}
...
with open(csvFile) as f:
    resultReader = csv.DictReader(f, skipinitialspace=True)
    data = [dict(row) for row in resultReader]

# Load list of field names
with open(csvFile) as f:
    for line in f:
