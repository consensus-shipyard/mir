...
import (
	...
	"github.com/filecoin-project/mir/samples/blockchain-chat-mobile/application"
)
...
var idToUserMap = map[int]string{
	0: "Alice",
	1: "Bob",
	2: "Charlie",
	3: "Donna",
...
func main() {
	...
	fmt.Printf("Starting node %d: %s\n", *nodeID, idToUserMap[*nodeID])
	fmt.Printf("Interceptor exposed on port: %d\n", 8080+*nodeID)
	fmt.Printf("Client server exposed on port: %d\n", 8180+*nodeID)
	fmt.Printf("Access client under: http://k3-mbp:8280/?user=%s\n", idToUserMap[*nodeID])
