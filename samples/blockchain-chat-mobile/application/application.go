...
import (
	...
	"github.com/filecoin-project/mir/samples/blockchain-chat-mobile/application/payloads"
	"github.com/filecoin-project/mir/samples/blockchain-chat-mobile/server"
	"google.golang.org/protobuf/types/known/timestamppb"
)

/**
* Application module - Mobile Chat Demo
* ==================
*
* Based on /samples/blockchain-chat/application/application.go
*
 */
...
func applyBlockToState(state *statepbtypes.State, block *blockchainpbtypes.Block) *statepbtypes.State {
	...
	if block.Payload.Message != "" {
		history = append(history, block.Payload)
	}
...
func (am *ApplicationModule) handleHeadChange(removedChain, addedChain, checkpointToForkRootChain []*blockchainpbtypes.Block, checkpointState *statepbtypes.State) error {
	...
	// update state in mobile server
	am.mobileServer.SetHistory(state.History)

	// print state
	fmt.Printf("=== STATE ===\n")
	for _, pl := range state.History {
		fmt.Println(pl.Message)
	}
	fmt.Printf("=============\nEnter new message: \n")
...
func (am *ApplicationModule) addNewMessage(message string) {
...
func NewApplication(nodeID t.NodeID, logger logging.Logger) modules.Module {
	...
	am.mobileServer = server.NewServer(":818"+string(nodeID), am.addNewMessage) // janky port def but should be ok as we will never have >10

	fmt.Println("===========================================")
	go am.mobileServer.Run()
	fmt.Println("===========================================")
