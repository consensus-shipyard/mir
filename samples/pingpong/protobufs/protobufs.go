...
func TimeoutEvent(destModule t.ModuleID) *eventpb.Event {
	return Event(
		destModule,
...
func PingMessage(destModule t.ModuleID, seqNr uint64) *messagepb.Message {
	return Message(
		destModule,
		&pingpongpb.Message{Type: &pingpongpb.Message_Ping{Ping: &pingpongpb.Ping{
			SeqNr: seqNr,
		}}},
	)
}
...
func PongMessage(destModule t.ModuleID, seqNr uint64) *messagepb.Message {
	return Message(
		destModule,
		&pingpongpb.Message{Type: &pingpongpb.Message_Pong{Pong: &pingpongpb.Pong{
			SeqNr: seqNr,
		}}},
	)
}
